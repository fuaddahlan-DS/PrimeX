...
//dd($member);
        ...
        return redirect()->route("newjob", ['id' => $cjvid, 'CategoryService' => 3]);
    }
    ...
    public function findUser(Request $request) {
        ...
        /*$result = Clientvehiclejoin::select('clientvehiclejoin.*', 'vehicles.RegistrationNo', 'clients.Name', 'clients.ContactNo')
                ->join('vehicles', 'vehicles.ID', '=', 'clientvehiclejoin.VehicleID')
                ->join('clients', 'clients.ID', '=', 'clientvehiclejoin.ClientID')
                ->Where(function ($query) use($input) {
                    if (!is_null($input['RegistrationNo']))
                        $query->where('vehicles.RegistrationNo', $input['RegistrationNo']);
                    if (!is_null($input['Name']))
                        $query->where('clients.Name', 'like', '%' . $input['Name'] . '%');
                    if (!is_null($input['ContactNo']))
                        $query->where('clients.ContactNo', '=', $input['ContactNo']);
        ...
        return redirect()->route("newjobbycash", ['id' => $cjvid, 'CategoryService' => 3]);
    }
    
    public function NewJobByCash($id, $CategoryService) {
    ...
    public function NewJob($id, $CategoryService) {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $sodid = Salesorderdetail::insertGetId(
                            [
                                ...
                                'UnitPrice' => $data['invoice_productNormalPrice'][$i],
                                ...
                                'GSTExempted' => 0,
                                'GST' => 0,
                                'Total' => $data['invoice_productTotal'][$i],
                            ]
            );
