...
class MemberController extends Controller
{
    ...
    public function updateMember(request $request){
    	...
    	try{
	    	...
	    	$client = DB::table('clients')
	    		...
	    		->where('clients.ID', $member->ClientID)
	    		->first();
	    	...
	    	if($client){
			    ...
			    if($checkEmail){
			    	...
			    	$id = DB::table('clients')->where('ID', $client->ID)->update(
			                [
			                    'Name' => $data['name'],
			                    'ContactNo' => $data['number'],
			                    'Email' => $data['email'],
			    	...
			    	$result = array(
		                'status' => true,
		                'message' => 'Member record ('.$client->Name.') successfully updated!',
		            );
			    }
		    }else{
		    	$result = array(
    	...
    	try{
	    	...
	    	if(!$car){
		    	...
		    	$id2 = DB::table('clientvehiclejoin')->insertGetId(
		                [
		                    'ClientID' => $data['memberID'],
		                    'VehicleID' => $id,
		                ]
		            );
		    	$result = array(
	                'status' => true,
	                'message' => 'New vehicle record ('.$data['number'].') successfully added!',
    ...
    public function addCarMemberDetails(request $request){
    	...
    	try{
	    	...
	    	if(!$car){
		    	...
		    	$result = array(
	                'status' => true,
	                'message' => 'New vehicle record ('.$data['number'].') successfully added!',
	                'data' => array('carNumber'=>$data['number'], 'carID'=>$id, 'ClientID'=>$member->ClientID),
	            );
		    }else{
		    	$result = array(
	                'status' => false,
    ...
    public function deleteCarDetails(request $request){
    	try{
	    	...
	    	$data = $request->all();
		    $clientvehiclejoin = DB::table('clientvehiclejoin')->where('VehicleID', $data['vehicleid'])->where('ClientID', $data['clientid'])->delete();

		    $vehicle = DB::table('vehicles')->where('ID', $data['vehicleid'])->delete();
